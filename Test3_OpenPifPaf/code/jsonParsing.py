...
if len(sys.argv)!=2:
    ...
    sys.exit(1)
files=os.listdir('test'+nb_video+'/') 
files.sort()
files.pop()
jsondata={'K': 10, 'njts': 17, 'frames': []}
imgname=files[0][:20]
for i in files:

    with open('test'+nb_video+'/'+i) as json_data:
    ...
    for j in d:
        ...
        n=0       
        while(k==0 and n<17):
            if(not (j['keypoints'][3*n]<200 and j['keypoints'][3*n]>10 and j['keypoints'][3*n+1]<895 and j['keypoints'][3*n+1]>280)):
                if(not (j['keypoints'][3*n]<1950 and j['keypoints'][3*n]>1600 and j['keypoints'][3*n+1]<895 and j['keypoints'][3*n+1]>300)):
                    if(not (j['keypoints'][3*n]==0 and j['keypoints'][3*n+1]==0)):

                        det={'cumscore': 0, 'pose2d': []}#'pose3d': [], 'angle_left': 0, 'angle_right': 0}
                        for k in range(17):
                            det['pose2d'].append(j['keypoints'][3*k])
                            #det['pose3d'].append(j['keypoints'][3*k])
                        for k in range(17):
                            det['pose2d'].append(j['keypoints'][3*k+1])
                            #det['pose3d'].append(j['keypoints'][3*k+1])
                        #for k in range(18):
                            #det['pose3d'].append(j['keypoints'][3*k+2])
    ...
    print(len(people))
    if(len(people)>=2):
        print('frame: ', len(jsondata['frames']))
...
with open('test'+nb_video+'/'+imgname+'.json', 'w') as outfile:
