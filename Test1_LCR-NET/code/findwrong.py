...
ax1.plot(kk['frames'],kk['z_rak'], marker='.', color='r', lw=0.2)
for i in range(len(k['frames'])):
    if k['bad_frame'][i]==0:
        ax1.text(kk['frames'][i],kk['z_lak'][i], str(k['bad_frame'][i]), color='g')
    else:
        ax1.text(kk['frames'][i],kk['z_rak'][i], str(k['bad_frame'][i]), color='#B22222')
...
ax4.plot(kk['frames'],kk['z_ras'], marker='.', color='r', lw=0.2)
for i in range(len(k['frames'])):
    if k['bad_frame'][i]==0:
        ax4.text(kk['frames'][i],kk['z_las'][i], str(k['bad_frame'][i]), color='g')
    else:
        ax4.text(kk['frames'][i],kk['z_ras'][i], str(k['bad_frame'][i]), color='#B22222')
...
if direction=='back':
    k.to_csv('testVedios'+'/test'+nb_video+'/'+number+'_LCR-NET_joints_3DKinect_back2.csv',encoding='gbk')
else:
    k.to_csv('testVedios'+'/test'+nb_video+'/'+number+'_LCR-NET_joints_3DKinect_front2.csv',encoding='gbk')
