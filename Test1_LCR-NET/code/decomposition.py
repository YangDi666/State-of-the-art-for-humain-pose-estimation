...
for i in files:
    ...
    if (len(re.findall('joints_3DKinect_.'+direction[1:]+'.csv', i))!=0):
        filecm=i
print(filecm, filegt)   
gt=pd.DataFrame(pd.read_csv('testVedios/test'+nb_video+'/'+filegt))
...
if direction=='back':
    kk.to_csv('testVedios'+'/test'+nb_video+'/'+number+'_LCR-NET_joints_3DKinect_back.csv',encoding='gbk')
else:
    kk.to_csv('testVedios'+'/test'+nb_video+'/'+number+'_LCR-NET_joints_3DKinect_front.csv',encoding='gbk')
